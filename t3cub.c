...
{
    //that function allows to choose which GPU to use
    ...
    //создание массивов для последующего копирования на GPU
...
#pragma acc data copyin(thermalConductivityMatrixMod[0:arrSize], thermalConductivityMatrixModCopy[0:arrSize]) copy(thermalConductivityMatrix[0:arrSize])
{
    ...
    while (error > minError && iteration < maxIterations) {
        ...
        //pragma to use data adress within host code ??? смотри Memo
        #pragma acc host_data use_device(thermalConductivityMatrix, thermalConductivityMatrixMod, thermalConductivityMatrixModCopy)
        {
            ...
            #pragma acc update host(thermalConductivityMatrixModCopy[errorIdx - 1]) //????????? Memo
            error = thermalConductivityMatrixModCopy[errorIdx - 1]; // -1 потому что функция из FORTRAN а там нумерация с 1 а не 0
        }
        ...
        //swapping matrices //в каком случае обмен указателей работает? Когда матрицы одного типа и размерности
